...
MV_ColMat_int::MV_ColMat_int(int m, int n) : v_(m * n), dim0_(m), dim1_(n), lda_(m), ref_(0) {}

MV_ColMat_int::MV_ColMat_int(int m, int n, const int& s) : v_(m * n), dim0_(m), dim1_(n), lda_(m), ref_(0) {
  operator=(s);
}
...
MV_ColMat_int::MV_ColMat_int(const MV_ColMat_int& m)
    : v_(m.dim0_ * m.dim1_), dim0_(m.dim0_), dim1_(m.dim1_), lda_(m.dim0_), ref_(0) {
  int M = m.dim0_;
  int N = m.dim1_;
...
MV_ColMat_int::MV_ColMat_int(int* d, int m, int n) : v_(m * n), dim0_(m), dim1_(n), lda_(m), ref_(0) {
  int mn = m * n;
...
MV_ColMat_int MV_ColMat_int::operator()(const MV_VecIndex& I, const MV_VecIndex& J) {
  ...
  if (I.end() >= dim0_ || J.end() >= dim1_) {
    std::cerr << "Matrix index: (" << I.start() << ":" << I.end() << "," << J.start() << ":" << J.end()
              << ") not a subset of (0:" << dim0_ - 1 << ", 0:" << dim1_ - 1 << ") "
              << "\n";
    exit(1);
  }
  ...
  //
  return MV_ColMat_int(
      &v_[J.start() * lda_ + I.start()],
      I.end() - I.start() + 1,
      J.end() - J.start() + 1,
      lda_,
      MV_Matrix_::ref
...
const MV_ColMat_int MV_ColMat_int::operator()(const MV_VecIndex& I, const MV_VecIndex& J) const {
