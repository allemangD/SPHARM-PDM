...
VECTOR_double ICPreconditioner_double::solve(const VECTOR_double& x) const {
  VECTOR_double y(x);
...
VECTOR_double ICPreconditioner_double::trans_solve(const VECTOR_double& x) const {
  VECTOR_double y(x);
...
static void ICFactor(const VECTOR_int& pntr, const VECTOR_int& indx, VECTOR_double& val) {
...
static void ICSolve(
    const VECTOR_int& pntr, const VECTOR_int& indx, const VECTOR_double& val, VECTOR_double& dest
) {
  int M = dest.size();
  ...
  F77NAME(dcscsm)
  (0, M, 1, 1, NULL, 1.0, descra, &val(0), &indx(0), &pntr(0), &dest(0), M, 0.0, &dest(1), M, &work(0), M);

  // lower diag transpose
  F77NAME(dcscsm)
  (1, M, 1, 1, NULL, 1.0, descra, &val(0), &indx(0), &pntr(0), &dest(0), M, 0.0, &dest(1), M, &work(0), M);
}
