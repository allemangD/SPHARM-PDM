...
template <class TYPE>
MV_Vector<TYPE>::MV_Vector(int n, const TYPE &v) : p_(new TYPE[n]), dim_(n), ref_(0) {
  if (p_ == NULL) {
    cerr << "Error: NULL pointer in MV_Vector(int) constructor " << endl;
    cerr << "       Most likely out of memory... " << endl;
    exit(1);
...
template <class TYPE>
MV_Vector<TYPE>::MV_Vector(const MV_Vector<TYPE> &m) : p_(new TYPE[m.dim_]), dim_(m.dim_), ref_(0) {
  if (p_ == NULL) {
...
//
template <class TYPE>
MV_Vector<TYPE>::MV_Vector(TYPE *d, int n, MV_Vector_::ref_type i) : p_(d), dim_(n), ref_(i) {}

template <class TYPE>
MV_Vector<TYPE>::MV_Vector(TYPE *d, int n) : p_(new TYPE[n]), dim_(n), ref_(0) {
  if (p_ == NULL) {
...
template <class TYPE>
MV_Vector<TYPE>::MV_Vector(const TYPE *d, int n) : p_(new TYPE[n]), dim_(n), ref_(0) {
  if (p_ == NULL) {
    cerr << "Error: Null pointer in MV_Vector(TYPE*, int) " << endl;
    exit(1);
  }
...
MV_Vector<TYPE> MV_Vector<TYPE>::operator()(const MV_VecIndex &I) {
  ...
  if (I.all()) {
    ...
    //
    if (I.end() >= dim_) {
      cerr << "MV_VecIndex: (" << I.start() << ":" << I.end() << ") too big for matrix (0:" << dim_ - 1
           << ") " << endl;
      exit(1);
    }
    return MV_Vector<TYPE>(p_ + I.start(), I.end() - I.start() + 1, MV_Vector_::ref);
  }
}
