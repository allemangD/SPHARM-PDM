...
MV_Vector_COMPLEX::MV_Vector_COMPLEX(int n) : p_(new COMPLEX[n]), dim_(n), ref_(0) {
  if (p_ == NULL) {
...
MV_Vector_COMPLEX::MV_Vector_COMPLEX(int n, const COMPLEX& v) : p_(new COMPLEX[n]), dim_(n), ref_(0) {
  if (p_ == NULL) {
    std::cerr << "Error: NULL pointer in MV_Vector_COMPLEX(int) constructor "
              << "\n";
    std::cerr << "       Most likely out of memory... "
...
MV_Vector_COMPLEX::MV_Vector_COMPLEX(const MV_Vector_COMPLEX& m)
    : p_(new COMPLEX[m.dim_]), dim_(m.dim_), ref_(0) {
  if (p_ == NULL) {
...
MV_Vector_COMPLEX::MV_Vector_COMPLEX(COMPLEX* d, int n) : p_(new COMPLEX[n]), dim_(n), ref_(0) {
  if (p_ == NULL) {
...
MV_Vector_COMPLEX::MV_Vector_COMPLEX(const COMPLEX* d, int n) : p_(new COMPLEX[n]), dim_(n), ref_(0) {
  if (p_ == NULL) {
    std::cerr << "Error: Null pointer in MV_Vector_COMPLEX(COMPLEX*, int) "
              << "\n";
    exit(1);
...
MV_Vector_COMPLEX MV_Vector_COMPLEX::operator()(void) { return MV_Vector_COMPLEX(p_, dim_, MV_Vector_::ref); }
...
MV_Vector_COMPLEX MV_Vector_COMPLEX::operator()(const MV_VecIndex& I) {
  ...
  if (I.all()) {
    ...
    return MV_Vector_COMPLEX(p_ + I.start(), I.end() - I.start() + 1, MV_Vector_::ref);
  }
}
