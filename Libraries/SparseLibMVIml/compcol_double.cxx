...
CompCol_Mat_double::CompCol_Mat_double(void) : val_(0), rowind_(0), colptr_(0), base_(0), nz_(0) {
  dim_[0] = 0;
  dim_[1] = 0;
}
...
CompCol_Mat_double::CompCol_Mat_double(const CompCol_Mat_double &S)
    : val_(S.val_), rowind_(S.rowind_), colptr_(S.colptr_), base_(S.base_), nz_(S.nz_) {
  dim_[0] = S.dim_[0];
  dim_[1] = S.dim_[1];
}
...
CompCol_Mat_double::CompCol_Mat_double(
    int M, int N, int nz, const VECTOR_double &_val, const VECTOR_int &r, const VECTOR_int &c, int _base
)
...
VECTOR_double CompCol_Mat_double::operator*(const VECTOR_double &x) const {
  int M = dim_[0];
  int N = dim_[1];
  ...
  F77NAME(dcscmm)
  (0, M, 1, N, 1.0, descra, &val_(0), &rowind_(0), &colptr_(0), &x(0), N, 1.0, &result(1), M, &work(1), M);
...
VECTOR_double CompCol_Mat_double::trans_mult(const VECTOR_double &x) const {
  int M = dim_[0];
  int N = dim_[1];

  //      Check for compatible dimensions:
  if (x.size() != M) {
    std::cerr << "Error in CompCol TransMatvec -- incompatible dimensions.\n";
    exit(1);
    return x;
  }
  ...
  F77NAME(dcscmm)
  (1, N, 1, M, 1.0, descra, &val_(0), &rowind_(0), &colptr_(0), &x(1), M, 1.0, &result(0), N, &work(0), N);
