...
CompRow_Mat_double::CompRow_Mat_double(void) : val_(0), rowptr_(0), colind_(0), base_(0), nz_(0) {
  dim_[0] = 0;
  dim_[1] = 0;
}
...
CompRow_Mat_double::CompRow_Mat_double(const CompRow_Mat_double &S)
    : val_(S.val_), rowptr_(S.rowptr_), colind_(S.colind_), base_(S.base_), nz_(S.nz_) {
  dim_[0] = S.dim_[0];
  dim_[1] = S.dim_[1];
}
...
CompRow_Mat_double::CompRow_Mat_double(
...
double &CompRow_Mat_double::set(int i, int j) {
  ...
  std::cerr << "Array element (" << i << "," << j << ") not in sparse structure -- cannot assign."
            << "\n";
  exit(1);
...
VECTOR_double CompRow_Mat_double::operator*(const VECTOR_double &x) const {
  ...
  F77NAME(dcsrmm)
  (0, M, 1, N, 1.0, descra, &val_(0), &colind_(0), &rowptr_(0), &x(1), N, 1.0, &result(0), M, &work(0), M);
...
VECTOR_double CompRow_Mat_double::trans_mult(const VECTOR_double &x) const {
  ...
  F77NAME(dcsrmm)
  (1, N, 1, M, 1.0, descra, &val_(0), &colind_(0), &rowptr_(0), &x(0), M, 1.0, &result(1), N, &work(1), N);
