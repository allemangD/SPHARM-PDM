...
Coord_Mat_double::Coord_Mat_double(void) : val_(0), rowind_(0), colind_(0), base_(0), nz_(0) {
  dim_[0] = 0;
  dim_[1] = 0;
}
...
Coord_Mat_double::Coord_Mat_double(const Coord_Mat_double &S)
    : val_(S.val_), rowind_(S.rowind_), colind_(S.colind_), base_(S.base_), nz_(S.nz_) {
  dim_[0] = S.dim_[0];
  dim_[1] = S.dim_[1];
}
...
VECTOR_double Coord_Mat_double::operator*(const VECTOR_double &x) const {
  ...
  F77NAME(dcoomm)
  (0, M, 1, N, 1.0, descra, &val_(0), &rowind_(0), &colind_(0), nz_, &x(1), N, 1.0, &result(1), M, &work(0), M
  );
...
VECTOR_double Coord_Mat_double::trans_mult(const VECTOR_double &x) const {
  ...
  F77NAME(dcoomm)
  (1, N, 1, M, 1.0, descra, &val_(0), &rowind_(0), &colind_(0), nz_, &x(1), M, 1.0, &result(1), N, &work(0), N
  );
  return result;
}
