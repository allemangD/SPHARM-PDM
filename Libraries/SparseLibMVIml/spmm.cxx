...
static void CoordMatVec_float(
  ...
  if (alpha == 1.0) {
    ...
    if (n == 1) {
      ...
      for (i = 0; i < n; i++) {
        for (j = 0; j < nnz; j++) {
          _SpMatVal(c, ldc, indx[j], i) += alpha * _SpMatVal(b, ldb, indx[j], i) * val[j];
        }
      }
    }
  }
...
static void CoordMatVec_double(
    int m,
    int n,
    int k,
    const double &alpha,
...
static void CompColMatVec_double(
    int m,
    int n,
    int k,
    const double &alpha,
  ...
  if (alpha == 1.0) {
    ...
    if (n == 1) {
      ...
      for (l = 0; l < n; l++) {
        for (i = 0; i < k; i++) {
          for (j = pntr[i]; j < pntr[i + 1]; j++) {
            _SpMatVal(c, ldc, indx[j], l) += alpha * _SpMatVal(b, ldb, i, l) * val[j];
          }
        }
      }
    }
...
static void CompColMatVec_float(
    int m,
    int n,
    int k,
    const float &alpha,
...
static void CompRowMatVec_double(
    int m,
    int n,
    int k,
    const double &alpha,
  ...
  if (alpha == 1.0) {
    ...
    if (n == 1) {
      ...
      for (l = 0; l < n; l++) {
        for (i = 0; i < m; i++) {
          for (j = pntr[i]; j < pntr[i + 1]; j++) {
            _SpMatVal(c, ldc, i, l) += alpha * _SpMatVal(b, ldb, indx[j], l) * val[j];
          }
        }
      }
    }
...
static void CompRowMatVec_float(
    int m,
    int n,
    int k,
    const float &alpha,
...
static void ScaleRectangularArray_double(int m, int n, double *c, int ldc, const double &beta) {
  int i, j;
...
static void ScaleRectangularArray_float(int m, int n, float *c, int ldc, const double &beta) {
  int i, j;
