...
SphericalHarmonicCoefficientFileReader::SphericalHarmonicCoefficientFileReader() { m_FileName = ""; }
...
void SphericalHarmonicCoefficientFileReader::Update() {
  ...
  if (fscanf(file, " { %d,", &count) != 1) {
    throw SphericalHarmonicCoefficientFileReaderException(
        __FILE__, __LINE__, "Error in the header of the Coef file"
    );
  }
  for (int i = 0; i < count; i++) {
    ...
    if (fscanf(file, format_3_real, &(elem[0]), &(elem[1]), &(elem[2])) != 3) {
      throw SphericalHarmonicCoefficientFileReaderException(
          __FILE__, __LINE__, "Error in the coefficient format"
      );
    }
  ...
  if (fscanf(file, "}") != 0) {
    throw SphericalHarmonicCoefficientFileReaderException(
        __FILE__, __LINE__, "Error in the end of the Coef file: '}' expected"
    );
  }
