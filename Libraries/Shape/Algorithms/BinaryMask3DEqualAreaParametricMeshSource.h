 ...
 *
 *  \brief This class provides a mesh source for both a surface mesh and the corresponding Spherical
 ...
 *  for hug objects (>100'000 vertices) this should be at least 5000 (go home and let it run
 * overnight...)
 *
 * InitParametricMesh: Optional initialization, can be used to continue on an earlier run of this
 * filter
 *
 * \par REFERENCE
 ...
 *  2. C. Brechbuhler: Description and Analysis of 3-D Shapes by Parametrization of Closed Surfaces,
 * 1995, Diss., IKT/BIWI, ETH Z¨urich, ISBN 3-89649-007-9.
 *
 * \par INPUT
...
class BinaryMask3DEqualAreaParametricMeshSourceException : public ExceptionObject {
 public:
  ...
  /** Constructor. */
  BinaryMask3DEqualAreaParametricMeshSourceException(
      const char* file,
      unsigned int line,
...
typedef Mesh<double, EqualAreaParametricMeshSourceMeshTypeDimension, EqualAreaParametricMeshSourceMeshTrait>
...
class ITK_EXPORT BinaryMask3DEqualAreaParametricMeshSource
 ...
 public:
  ...
  virtual void SetInitParametricMesh(OutputMeshType* in) {
    m_InitParametricMesh = in;
    m_InitParametricMeshSet = true;
  };
