 ...
 * Option: UseScalingOn/Off() disables scaling matching with Procrustes (does not disable scaling
 * normalization of the mean shape) (on by default)
 *
...
class Mesh3DProcrustesAlignFilter : public ProcessObject {
 public:
  ...
  TransformType* GetTransform(unsigned int idx) { return m_MeshTransform[idx]->GetTransform(); }
  ...
  void SetUseScalingOn() { this->SetUseScaling(true); }

  /** Normalization with Scaling off */
  void SetUseScalingOff() { this->SetUseScaling(false); }
  ...
  void SetUseInitialAverageOn() { this->SetUseInitialAverage(true); }

  /** do not use average as reference for initialization. The first surface will be used
   * instead.(default) */
  void SetUseInitialAverageOff() { this->SetUseInitialAverage(false); }
  ...
  void SetUseNormalizationOn() { this->SetUseNormalization(true); }

  void SetUseNormalizationOff() { this->SetUseNormalization(false); }

  itkSetMacro(UseNormalization, bool);
  itkGetMacro(UseNormalization, bool);

  /** Only do one iteration */
  void SetUseSingleIterationOn() { this->SetUseSingleIteration(true); }

  /** Iterate until convergence (default)*/
  void SetUseSingleIterationOff() { this->SetUseSingleIteration(false); }
  ...
  void SetAlignRotationOn() { this->SetAlignRotation(true); }

  void SetAlignRotationOff() { this->SetAlignRotation(false); }
 ...
 protected:
   ...
   */
  TransformPointer GetProcrustesMatch(
      unsigned int idx, OutputMeshPointer targetMesh, TranslationType targetCenter
  );
