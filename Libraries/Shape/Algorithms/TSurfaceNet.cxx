...
TVertexItem* TVertexList::search(TVertex val) const {
  ...
  forall_vertexlistitems(p, *this) if (p->val_ == val) { break; }
...
int TVertexList::contains(TVertex val) const {
  ...
  forall_vertexlistitems(p, *this) if (p->val_ == val) { return 1; }
...
int TVertexList::size() const {
  ...
  forall_vertexlistitems(a, *this) i++;
...
int TSurfaceNet::nface() const {
  ...
  for (int i = 0; i < va_->size(); ++i) {
    ...
    for (int j = 0; j + 1 < v.count_; j += 2) {
      ...
      if (v.neighb_[j] > i && v.neighb_[j + 1] > i && v.neighb_[second] > i) {
        nfaces++;
      }
    }
  }
...
void TNetExtractor::encode(
    int nx, int ny, int /*UNUSED nz*/, int& number, unsigned char* nbc, numentry* num, int z
) {
  for (int y = 0; y < ny; y++) {
    for (int x = 0; x < nx; x++) {
...
TVertexList* TNetExtractor::extractnet() {
  ...
  for (z = 0; z < nz - 1; z++) {
    numentry* help_i = num[0];
    num[0] = num[1];
    num[1] = num[2];
    num[2] = help_i;
    unsigned char* help_u = nbc_this;
    nbc_this = nbc_next;
    nbc_next = help_u;
        ...
        if (nc) {
          for (i = 0; i < pt1[nc].count; i++) {
            ...
            if (nc_act) {
              ...
              // the values where to look for the neighbor's numbers in the num array are
              // precomputed and stored in table pt2
              for (k = 0; k < count2; k++) {
                ...
                // numentry dbg = num[1 + z_tab[dir1] + z_tab[dir2]][xy + xy_tab[dir1] +
                // xy_tab[dir2]];
                num_act = num[1 + z_tab[dir1] + z_tab[dir2]][xy + xy_tab[dir1] + xy_tab[dir2]].nums[t_ne2];
                neighb_act[count_act++] = num_act;
              }
