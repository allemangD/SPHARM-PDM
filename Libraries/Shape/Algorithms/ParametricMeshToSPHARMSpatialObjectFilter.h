 ...
 *
 * \par PARAMETERS
 *   FlipTemplate : Needed for the parameter alignment based on the first order ellipsoid (1 order
 * SPHARM coefficient). When establishing correspondence with SPHARM, then it is essential to
 * provide the same FlipTemplate to all surfaces that are compared.
 *
 * \par REFERENCE
 *  1. C. Brechbuhler, G. Gerig, and O. Kubler: Parametrization of closed surfaces for 3-D shape
 ...
 * optimal for encoding the SPHARM coefficients when it is preserving the area ratio of surface
 * elements from the object surface to the parametrization surface.
 *
 *  */
...
class ParametricMeshToSPHARMSpatialObjectFilterException : public itk::ExceptionObject {
 public:
  ...
  /** Constructor. */
  ParametricMeshToSPHARMSpatialObjectFilterException(
      const char *file, unsigned int line, const char *message = "Error in generating SPHARM coeffs"
  )
  ...
  /** Constructor. */
  ParametricMeshToSPHARMSpatialObjectFilterException(
      const std::string &file, unsigned int line, const char *message = "Error in generating SPHARM coeffs"
  )
      : ExceptionObject(file, line) {
    SetDescription(message);
  }
...
class ITK_EXPORT ParametricMeshToSPHARMSpatialObjectFilter : public itk::ProcessObject {
 public:
  ...
  /** returns the Parametrization Mesh. Spherical vertices correspond to Surface vertices of same
   * index */
  ...
  /** Fliptemplate in order to align the parametrization, without a flipTemplate it is difficult to
      compare different SPHARM objects, as the correspondence can be flipped at any of the axis
     (since the first order ellipsoid is ambiguosly defined */
   ...
   */
  void SetParametrizationFlipIndex(int flipIndex) { m_FlipParametrizationIndex = flipIndex; }
  ...
  virtual itk::ProcessObject::DataObjectPointer MakeOutput(
 ...
 protected:
  ...
  /** adjust coefficients such that first order ellipsoid axis build a right hand coordinate
   * system*/
