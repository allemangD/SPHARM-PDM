...
struct IteratorSurfaceVertex {
  int x, y, z, count, neighb[14];
};

struct IteratorSurfaceNet {
  int nvert, nface, *face;
  IteratorSurfaceVertex *vert;
};
...
typedef struct {
  int max_active,  // max. #of active inequalities
      print_itn;   // iter #for debug output; <0 : never
  //    reread_param;                        // bool: re-read every 20 itns
  double delta,                                  // step for finite differences
...
class EqualAreaParametricMeshNewtonIterator {
 ...
 public:
  EqualAreaParametricMeshNewtonIterator(
      const IteratorSurfaceNet &netref, EqualAreaParametricMeshParameter &par
  );
  EqualAreaParametricMeshNewtonIterator(
      const IteratorSurfaceNet &netref, EqualAreaParametricMeshParameter &par, double *initPar
  );
  ~EqualAreaParametricMeshNewtonIterator();
  double iterate();
