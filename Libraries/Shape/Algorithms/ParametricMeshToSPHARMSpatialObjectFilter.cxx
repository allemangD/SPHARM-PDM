...
ParametricMeshToSPHARMSpatialObjectFilter::ParametricMeshToSPHARMSpatialObjectFilter() {
  ...
  SpatialObjectType::Pointer output = dynamic_cast<SpatialObjectType *>(this->MakeOutput(0).GetPointer());
...
ParametricMeshToSPHARMSpatialObjectFilter::DataObjectPointer ParametricMeshToSPHARMSpatialObjectFilter::
...
void ParametricMeshToSPHARMSpatialObjectFilter::SetOutput(SpatialObjectType *output) {
  itkWarningMacro(
      << "SetOutput(): This method is slated to be removed from ITK.  Please use GraftOutput() in "
         "possible combination with DisconnectPipeline() instead."
  );
  this->ProcessObject::SetNthOutput(0, output);
}

void ParametricMeshToSPHARMSpatialObjectFilter::GraftOutput(SpatialObjectType *graft) {
...
void ParametricMeshToSPHARMSpatialObjectFilter::GenerateInputRequestedRegion() {
...
void ParametricMeshToSPHARMSpatialObjectFilter::SetInputParametrizationMesh(InputMeshType *mesh) {
#ifdef _MSC_VER
...
void ParametricMeshToSPHARMSpatialObjectFilter::SetInputSurfaceMesh(InputMeshType *mesh) {
#ifdef _MSC_VER
...
int ParametricMeshToSPHARMSpatialObjectFilter::flip_ll(double n) { return (int)sqrt((double)(n - 1)); }
...
double ParametricMeshToSPHARMSpatialObjectFilter::flip_fu2(double n) { return pow(-1.0, flip_m2(n)); }
...
double ParametricMeshToSPHARMSpatialObjectFilter::FlipAxesDistance(
    const CoefListType *coef1, const CoefListType *coef2
) {
  double diff[3];
  double sum = 0;
...
double ParametricMeshToSPHARMSpatialObjectFilter::CoefDistance(
    const CoefListType *coef1, const CoefListType *coef2
) {
  double diff[3];
  double sum = 0;
...
void ParametricMeshToSPHARMSpatialObjectFilter::EllipseScaleAlign(
    const CoefListType *incoef, CoefListType *outcoef
) {
  outcoef->clear();
  // normalize size relative to volume = 4/3 Pi a b c
  double a = sqrt(
      ((*incoef)[1])[0] * ((*incoef)[1])[0] + ((*incoef)[1])[1] * ((*incoef)[1])[1] +
      ((*incoef)[1])[2] * ((*incoef)[1])[2]
  );
  double b = sqrt(
      ((*incoef)[2])[0] * ((*incoef)[2])[0] + ((*incoef)[2])[1] * ((*incoef)[2])[1] +
      ((*incoef)[2])[2] * ((*incoef)[2])[2]
  );
  double c = sqrt(
      ((*incoef)[3])[0] * ((*incoef)[3])[0] + ((*incoef)[3])[1] * ((*incoef)[3])[1] +
      ((*incoef)[3])[2] * ((*incoef)[3])[2]
  );
...
void ParametricMeshToSPHARMSpatialObjectFilter::EllipseAlign(
    const CoefListType *incoef, CoefListType *outcoef
) {
...
void ParametricMeshToSPHARMSpatialObjectFilter::EllipseAlign(
    const CoefListType *incoef, CoefListType *outcoef, double scale
) {
  outcoef->clear();
  // scale
  for (unsigned int i = 0; i < incoef->size(); i++) {
  ...
  double a = sqrt(
      ((*outcoef)[1])[0] * ((*outcoef)[1])[0] + ((*outcoef)[1])[1] * ((*outcoef)[1])[1] +
      ((*outcoef)[1])[2] * ((*outcoef)[1])[2]
  );
  double b = sqrt(
      ((*outcoef)[2])[0] * ((*outcoef)[2])[0] + ((*outcoef)[2])[1] * ((*outcoef)[2])[1] +
      ((*outcoef)[2])[2] * ((*outcoef)[2])[2]
  );
  double c = sqrt(
      ((*outcoef)[3])[0] * ((*outcoef)[3])[0] + ((*outcoef)[3])[1] * ((*outcoef)[3])[1] +
      ((*outcoef)[3])[2] * ((*outcoef)[3])[2]
  );
  // rotation axes
  ...
  // std::cout << "c" << (*incoef)[1][0] << "," <<  (*incoef)[1][1] << "," << (*incoef)[1][2] <<
  // std::endl; std::cout << "c" << (*incoef)[2][0] << "," <<  (*incoef)[2][1] << "," <<
  // (*incoef)[2][2] << std::endl; std::cout << "c" << (*incoef)[3][0] << "," <<  (*incoef)[3][1] <<
  // "," << (*incoef)[3][2] << std::endl; std::cout <<  std::endl; rotNormTRS[coef_] :=
  // coef(x,y,z).{{e1,e2,e3}; (1x3) (3x3) ->(1x3) x * e11 + y * e12 + z * e13 ...
  ...
  // std::cout << "c" << (*outcoef)[1][0] << "," <<  (*outcoef)[1][1] << "," << (*outcoef)[1][2] <<
  // std::endl; std::cout << "c" << (*outcoef)[2][0] << "," <<  (*outcoef)[2][1] << "," <<
  // (*outcoef)[2][2] << std::endl; std::cout << "c" << (*outcoef)[3][0] << "," <<  (*outcoef)[3][1]
  // << "," << (*outcoef)[3][2] << std::endl; std::cout <<  std::endl;
}
...
void ParametricMeshToSPHARMSpatialObjectFilter::MakeEllipsoidAxisRightHand(CoefListType *coef) {
  double a = sqrt(
      ((*coef)[1])[0] * ((*coef)[1])[0] + ((*coef)[1])[1] * ((*coef)[1])[1] +
      ((*coef)[1])[2] * ((*coef)[1])[2]
  );
  double b = sqrt(
      ((*coef)[2])[0] * ((*coef)[2])[0] + ((*coef)[2])[1] * ((*coef)[2])[1] +
      ((*coef)[2])[2] * ((*coef)[2])[2]
  );
  double c = sqrt(
      ((*coef)[3])[0] * ((*coef)[3])[0] + ((*coef)[3])[1] * ((*coef)[3])[1] +
      ((*coef)[3])[2] * ((*coef)[3])[2]
  );
...
void ParametricMeshToSPHARMSpatialObjectFilter::BestFlip(
    CoefListType *coef, const CoefListType *flipTemplate
) {
...
int ParametricMeshToSPHARMSpatialObjectFilter::BestFlipAlign(
    CoefListType *coef, const CoefListType *flipTemplate
) {
  ...
  if (curMatch < bestFlipMatch) {
    bestFlipMatch = curMatch;
    bestFlipCoef = curflipCoef;
    bestFlipIndex = 1;
  }
...
ParametricMeshToSPHARMSpatialObjectFilterLegendre::ParametricMeshToSPHARMSpatialObjectFilterLegendre(
  ...
  for (l = 2; l <= L; l++) {
    ...
    // cout << "l" << l << ":";
    for (j = l; j > 0; j--) {
      ((plm[l])[0])[j] = (n1 * ((plm[l - 1])[0])[j - 1] - n2 * ((plm[l - 2])[0])[j]) / (double)l;
      // cout << " " << ((plm[l])[0])[j];
    }
...
void ParametricMeshToSPHARMSpatialObjectFilterLegendre::ylm(
    int l, int m, float x, float y, float z, double &re, double &im
) {
  int i;
  double p, p_ptr[256];
...
void ParametricMeshToSPHARMSpatialObjectFilterLegendre::Ylm(
    int l, int m, float theta, float phi, double &re, double &im
) {
...
int ParametricMeshToSPHARMSpatialObjectFilter::Get_BaseVal(
    PointsContainerPointer paraPoints, float *&A, int &nvert, int &degree
) {
  ...
  for (int i = 0; i < nvert; i++) {
    ...
    vert[3 * i + 0] = curPoint[0];
    vert[3 * i + 1] = curPoint[1];
    vert[3 * i + 2] = curPoint[2];
  }
...
void ParametricMeshToSPHARMSpatialObjectFilter::ComputeCoeffs() {
  ...
  if (surfPoints->Size() != paraPoints->Size()) {
    throw ParametricMeshToSPHARMSpatialObjectFilterException(
        __FILE__,
        __LINE__,
        "ParametricMeshToSPHARMSpatialObjectFilter: surface and para do not have same number of "
        "points"
    );
  }
  ...
  Eigen::MatrixXf A_mat;
  {
    ...
    if (m_int < n_int) {
      ...
      throw ParametricMeshToSPHARMSpatialObjectFilterException(
          __FILE__, __LINE__, "Not enough points; cannot solve SPHARM coefficients."
      );
    }
  ...
  // householderQr is closer to the LAPACK sgels implementation, however solving as a normal
  // equation is about 3x faster Some potential for instabilities in pathological cases. See
  // https://eigen.tuxfamily.org/dox/group__LeastSquares.html x_mat =
  // A_mat.householderQr().solve(y_mat);
...
void ParametricMeshToSPHARMSpatialObjectFilter::RotateParametricMesh() {
  ...
  //  std::cout << "values " << ev[sortIndex[0]] <<"," << ev[sortIndex[1]] <<"," << ev[sortIndex[2]]
  //  <<"," << std::endl;
...
/** Generate the data */
void ParametricMeshToSPHARMSpatialObjectFilter::GenerateData() {
  if (this->GetNumberOfInputs() < 2) {
    throw ParametricMeshToSPHARMSpatialObjectFilterException(
        __FILE__,
        __LINE__,
        "ParametricMeshToSPHARMSpatialObjectFilter: 2 surfaces are needed, a surface and a "
...
/** PrintSelf */
void ParametricMeshToSPHARMSpatialObjectFilter::PrintSelf(std::ostream &os, itk::Indent indent) const {
