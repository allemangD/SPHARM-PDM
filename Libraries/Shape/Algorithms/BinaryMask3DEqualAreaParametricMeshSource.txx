...
BinaryMask3DEqualAreaParametricMeshSource<TInputImage>::BinaryMask3DEqualAreaParametricMeshSource() {
...
template <class TInputImage>
void BinaryMask3DEqualAreaParametricMeshSource<TInputImage>::SetInput(const InputImageType *image) {
  this->ProcessObject::SetNthInput(0, const_cast<InputImageType *>(image));
}

/** Generate the data */
template <class TInputImage>
void BinaryMask3DEqualAreaParametricMeshSource<TInputImage>::GenerateData() {
  ...
  InputImageConstPointer image = static_cast<const InputImageType *>(this->ProcessObject::GetInput(0));
  ...
  if (minMaxCalc->GetMinimum() >= minMaxCalc->GetMaximum()) {
    throw BinaryMask3DEqualAreaParametricMeshSourceException(
        __FILE__, __LINE__, "BinaryMask3DEqualAreaParametricMeshSource : Image empty"
    );
  }
  ...
  for (int i = 0; i < net.nvert; i++) {
    for (int j = 0; j < net.vert[i].count - 1; j += 2) {
      ...
      if (net.vert[i].neighb[j] > i &&  // 1st corner creates a square
          net.vert[i].neighb[j + 1] > i && net.vert[i].neighb[second] > i) {
        *curface++ = i;
  ...
  if (m_EulerNum != 2) {
    ...
    sprintf(
        message,
        "Warning: Euler equation not satisfied. Euler Number : %d - %d = %d",
        net.nvert,
        net.nface,
        m_EulerNum
    );
  ...
  // C. Brechbühler,  "Constrained Optimization",  Swiss Federal Institute of Technology,
  // Communication Technology Laboratory, Image Science, BIWI-TR-166, 1995
  // ftp://ftp.vision.ee.ethz.ch/publications   echreports/eth_biwi_00034.ps.gz
  par.max_active = 1000;
  par.print_itn = -2;
  par.delta = 3e-7;
  par.constr_tol = 1e-3;
  ...
  if (m_InitParametricMeshSet) {
    ...
    for (int i = 0; i < net.nvert; i++) {
      ...
      xvec[3 * i + 0] = curPoint[0];
      xvec[3 * i + 1] = curPoint[1];
      xvec[3 * i + 2] = curPoint[2];
    }
  ...
  // map IteratorSurfaceNet to ITK Mesh for surfaceMesh
  {
    ...
    for (int i = 0; i < net.nvert; i++) {
      ...
      index[0] = (net.vert[i]).x;
      index[1] = (net.vert[i]).y;
      index[2] = (net.vert[i]).z;
...
/** PrintSelf */
template <class TInputImage>
void BinaryMask3DEqualAreaParametricMeshSource<TInputImage>::PrintSelf(std::ostream &os, Indent indent)
    const {
  Superclass::PrintSelf(os, indent);

  os << indent << "ObjectValue: " << static_cast<NumericTraits<unsigned char>::PrintType>(m_ObjectValue)
     << std::endl;

  os << indent
     << "NumberOfInterations: " << static_cast<NumericTraits<unsigned int>::PrintType>(m_NumberOfIterations)
     << std::endl;
}
