...
EqualAreaParametricMeshSparseMatrix::EqualAreaParametricMeshSparseMatrix(
    int maxRow, int maxCol, int maxNonzero
) {
  max_col = maxCol;
  max_row = maxRow;
...
void EqualAreaParametricMeshSparseMatrix::test_matrix() {
  ...
  // assert(max_nonzero >= 14, "Not enough non-zeros in
  // EqualAreaParametricMeshSparseMatrix::test_matrix");
  n_row = 4;
  n_col = 7;
...
int EqualAreaParametricMeshNewtonIterator::check_constraints(
    int n_equal,
    int n_inequal, /*int *active,*/
    double *inequal,
    /* double* c_hat,*/
    double *c_hat_try,
    double &badness
...
void EqualAreaParametricMeshNewtonIterator::activate_initial(
    const int n_equal, const int n_inequal, const double *inequal
) {
...
void EqualAreaParametricMeshNewtonIterator::constraints(
    const IteratorSurfaceNet & /* net */, const double *_x /* x */, double *equal, double *inequal
) {
...
double EqualAreaParametricMeshNewtonIterator::one_inequality(
    const IteratorSurfaceNet & /* net */, const double *x_ /* x */, int which
) {
  double sines[4];
...
void EqualAreaParametricMeshNewtonIterator::write_YSMP(
    const char *name, int n_row, int n_col, int *ia, int *ja, double *a, int append
) {
...
void EqualAreaParametricMeshNewtonIterator::write_vector(
    const char *name, int n, double *vector, int append
) {
  if (append) {
...
double EqualAreaParametricMeshNewtonIterator::aug_lagrangian(
    double step, int &_activate, double &badness, double &c_sqr_sum, double *c_hat_try
) {
...
void EqualAreaParametricMeshNewtonIterator::generate_matrix(
    struct CompRows *mat, const IteratorSurfaceNet &_net
) {
...
void EqualAreaParametricMeshNewtonIterator::modify_matrix(
    struct CompRows *mat, const IteratorSurfaceNet &_net
)
...
void EqualAreaParametricMeshNewtonIterator::set_longi_rhs(
    const IteratorSurfaceNet &_net, double *rhs, double *lati
) {
...
void EqualAreaParametricMeshNewtonIterator::start_values(const IteratorSurfaceNet &_net, double *cartesian) {
  ...
  // call of compressed row matrix = nxn with n_nonzero items that are non-zero, mat.a = value
  // vector, mat.ia = row pointer, mat.ja = column index Build up the structure from a grid in
  // 0-relative compressed row format
...
double EqualAreaParametricMeshNewtonIterator::spher_area4(
    const double *x, const int corner[4], double spat[4]
) {
  const double *a = x + 3 * corner[0], *b = x + 3 * corner[1], *c = x + 3 * corner[2], *d = x + 3 * corner[3],
               ab = dotproduct3(a, b), ac = dotproduct3(a, c), ad = dotproduct3(a, d), bc = dotproduct3(b, c),
               bd = dotproduct3(b, d), cd = dotproduct3(c, d), Ca = bd - ad * ab, Cb = ac - ab * bc,
               Cc = bd - bc * cd, Cd = ac - cd * ad;
...
void EqualAreaParametricMeshNewtonIterator::spher_step(
    double step, double *src, double *vec, int nvect, double *dest
) {
  for (int i = 3 * nvect; i--;) {
