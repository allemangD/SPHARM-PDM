 ...
 *
 *  \brief This class provides convenient class for generating 3D meshes represented by Spherical
 * Harmonics descriptor and the medial axis associated.
 *
...
class SphericalHarmonicMedialAxisMeshSourceException : public itk::ExceptionObject {
 public:
  ...
  /** Constructor. */
  SphericalHarmonicMedialAxisMeshSourceException(
      const char* file,
      unsigned int line,
...
class SphericalHarmonicMedialAxisMeshSource
    : public itk::MeshSource<SphericalHarmonicMedialAxisMeshSourceMeshType> {
 public:
  ...
  vtkSmartPointer<vtkPolyData> GetOutputMedialAxis() { return m_outputMedialAxis; }
 ...
 protected:
  ...
  //
  // 			void set_up_icosahedron_triangs(Point3* all_vert, Point3* all_triangs, int subdiv,
  // int n_vert, int n_phi, int n_theta,
  // double *icos, int n_triangs,
  // int *triangs);
  //
  // 			void interpol_vert(int n_phi, int n_theta, Point3 *mesh, int n_vertex, double
  // *icos, Point3 *vertex);
  //
  // 			int mod(int x, int y);
  //
  // 			void interpol_2d(int n_phi, int n_theta, int xd, int xu, int yd, int yu, double
  // ksi,
  // double eta, Point3 *mesh, 								  double *xi, double
  // *yi, double *zi);
