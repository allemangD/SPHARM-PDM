...
class TVertex {
 public:
  ...
  friend float vdist(const TVertex& v1, const TVertex& v2) {
    return sqrt(
        (double)(v1.wh_[0] - v2.wh_[0]) * (v1.wh_[0] - v2.wh_[0]) +
        (v1.wh_[1] - v2.wh_[1]) * (v1.wh_[1] - v2.wh_[1]) + (v1.wh_[2] - v2.wh_[2]) * (v1.wh_[2] - v2.wh_[2])
    );
  }
...
class TVertexArray {
 public:
  ...
  TVertex& operator[](int ix) { return ia_[ix]; }

  const TVertex& operator[](int ix) const { return ia_[ix]; }
