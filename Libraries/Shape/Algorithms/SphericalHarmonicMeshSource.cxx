...
static double vert[12][3] = {
    ...
    {Z, -X, 0.0},
    {-Z, -X, 0.0}};
static int triang[20][3] = {{0, 4, 1}, {0, 9, 4},  {9, 5, 4},  {4, 5, 8},  {4, 8, 1},  {8, 10, 1}, {8, 3, 10},
                            {5, 3, 8}, {5, 2, 3},  {2, 7, 3},  {7, 10, 3}, {7, 6, 10}, {7, 11, 6}, {11, 0, 6},
                            {0, 1, 6}, {6, 1, 10}, {9, 0, 11}, {9, 11, 2}, {9, 2, 5},  {7, 2, 11}};
static int edge[30][2] = {{0, 1},  {0, 4},  {0, 6},  {0, 9},  {0, 11}, {1, 4},  {1, 6}, {1, 8},
...
SphericalHarmonicMeshSource::~SphericalHarmonicMeshSource() { delete[] m_icos; }
...
void SphericalHarmonicMeshSource::GenerateData() {
  ...
  if (m_Level <= 0) {
    throw SphericalHarmonicPolynomialException(
        __FILE__, __LINE__, "Subdivision level must be greater than zero."
    );
  }
  ...
  set_up_icosahedron_triangs(
      all_vert, all_triangs, m_Level, n_vert, n_phi, n_theta, m_icos, n_triag, triangs
  );
...
void SphericalHarmonicMeshSource::set_up_icosahedron_triangs(
    Point3 *all_vert,
    Point3 *all_triangs,
    int subdiv,
    int n_vert,
  ...
  for (i = 0; i < 30; i++) {
    ...
    for (n = 1; n < subdiv; n++) {
      ...
      length = sqrt(
          (double)all_vert[k][0] * all_vert[k][0] + all_vert[k][1] * all_vert[k][1] +
  ...
  if (subdiv == 1) {
    ...
    for (i = 0; i < numtriags; i++) {
      if (triangs[i] == -1) {
        std::cerr << " - " << i << " :" << all_triangs[i][0] << "," << all_triangs[i][1] << ","
                  << all_triangs[i][2] << std::endl;
      }
    }
  ...
  for (i = 0; i < n_vert; i++) {
    ...
    icos[2 * i + 1] =
        atan(
            all_vert[i][2] / sqrt((double)all_vert[i][0] * all_vert[i][0] + all_vert[i][1] * all_vert[i][1])
        ) +
        M_PI_2;
...
void SphericalHarmonicMeshSource::interpol_vert(
    int n_phi, int n_theta, Point3 *mesh, int n_vertex, double *icos, Point3 *vertex
) {
  int i;
  double phi, theta;
  double x, y;
...
void SphericalHarmonicMeshSource::interpol_2d(
    int n_phi,
    int /* n_theta */,
    int xd,
    int xu,
  ...
  *xi = f00 + (f10 - f00) * ksi + (f01 - f00) * eta + (f11 + f00 - f10 - f01) * ksi * eta;
  ...
  *yi = f00 + (f10 - f00) * ksi + (f01 - f00) * eta + (f11 + f00 - f10 - f01) * ksi * eta;
  ...
  *zi = f00 + (f10 - f00) * ksi + (f01 - f00) * eta + (f11 + f00 - f10 - f01) * ksi * eta;
}
