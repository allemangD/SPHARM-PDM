...
void SphericalHarmonicPolynomial<TDimension>::SetCoefs(CoefListType &coeflist) {
  ...
  if (!(TDimension == d)) {
    throw SphericalHarmonicPolynomialException(
        __FILE__,
        __LINE__,
        "The dimension of the input coefficient list does not match the specified dimension!"
    );
  }
  m_Coefs = coeflist;
}
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::Evaluate(
    unsigned int from_l, unsigned int to_l, double phi, double theta, double *sum
) {
  // if(m_Dimension != 2)
  //  throw SphericalHarmonicPolynomialException(__FILE__, __LINE__, "Only Spherical Harmonics of dimension
  //  two is
  // currently implemented.");
  // if(size > TDimension)
  //   throw SphericalHarmonicPolynomialException(__FILE__, __LINE__, "Dimension of the output sum exceeds the
  //   dimension
  // of the coefficients.");
  if (m_Coefs.empty()) {
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::EvaluateAt(
    unsigned int l, unsigned int m, double phi, double theta, double *result
) {
  if (m_Coefs.empty()) {
    throw SphericalHarmonicPolynomialException(__FILE__, __LINE__, "Coefficients must be specified.");
  }
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::EvaluateDTheta(
    unsigned int from_l, unsigned int to_l, double phi, double theta, double *sum
) {
  if (m_Coefs.empty()) {
    throw SphericalHarmonicPolynomialException(__FILE__, __LINE__, "Coefficients must be specified.");
  ...
  if (to_l > m_Degree) {
    throw SphericalHarmonicPolynomialException(
        __FILE__, __LINE__, "The evaluated degree mustn't exceed the size of the coefficients."
    );
  }
  if (m_Coefs.size() < (to_l + 1) * (to_l + 1)) {
    throw SphericalHarmonicPolynomialException(__FILE__, __LINE__, "Coefficients size mismatch.");
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::EvaluateDPhi(
    unsigned int from_l, unsigned int to_l, double phi, double theta, double *sum
) {
  if (m_Coefs.empty()) {
    throw SphericalHarmonicPolynomialException(__FILE__, __LINE__, "Coefficients must be specified.");
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::EvaluateDDThetaTheta(
    unsigned int from_l, unsigned int to_l, double phi, double theta, double *sum
) {
  if (m_Coefs.empty()) {
    throw SphericalHarmonicPolynomialException(__FILE__, __LINE__, "Coefficients must be specified.");
  ...
  for (l = (int)from_l; l <= (int)to_l; l++)  // for the from_l degree to to_l degree
  {
    for (m = 0; m <= l; m++) {
      ...
      for (coord = 0; coord < (int)TDimension; coord++) {
        ...
        realPart =
            eval0[0] * temp1 + (eval1[0] * cosPhi + eval1[1] * sinPhi) * temp2 + realLastTerm * factor2;

        imPart = eval0[1] * temp1 + (eval1[1] * cosPhi - eval1[0] * sinPhi) * temp2 + imLastTerm * factor2;
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::EvaluateDDThetaPhi(
    unsigned int from_l, unsigned int to_l, double phi, double theta, double *sum
) {
  if (m_Coefs.empty()) {
    throw SphericalHarmonicPolynomialException(__FILE__, __LINE__, "Coefficients must be specified.");
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::EvaluateDDPhiPhi(
    unsigned int from_l, unsigned int to_l, double phi, double theta, double *sum
) {
  if (m_Coefs.empty()) {
    throw SphericalHarmonicPolynomialException(__FILE__, __LINE__, "Coefficients must be specified.");
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::GetCoefAt(
    unsigned int l, unsigned int m, unsigned int coord, double *coef
) {
  if (m == 0) {
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::GetUnitNormal(
    unsigned int from_l, unsigned int to_l, double phi, double theta, double *n
) {
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::EvaluateFirstFundamentalForm(
    unsigned int from_l, unsigned int to_l, double phi, double theta, double *result
) {
  double dtheta[3], dphi[3];
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::EvaluateSecondFundamentalForm(
    unsigned int from_l, unsigned int to_l, double phi, double theta, double *result
) {
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::GetPrincipalCurvatures(
    unsigned int from_l, unsigned int to_l, double phi, double theta, double *kappa
) {
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::GetPrincipalDirectionsUV(
    unsigned int from_l, unsigned int to_l, double phi, double theta, double *uv
) {
  double first[3], second[3];
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::GetPrincipalDirections(
    unsigned int from_l, unsigned int to_l, double phi, double theta, double *dir
) {
  double uv[4];
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::ComputePrincipalCurve(
    unsigned int from_l,
    unsigned int to_l,
    double startPhi,
    double startTheta,
...
template <unsigned int TDimension>
double SphericalHarmonicPolynomial<TDimension>::ComputeC(
    unsigned int from_l, unsigned int to_l, double phi, double theta
) {
  double kappa[2];
  double r;
  double c;
...
template <unsigned int TDimension>
double SphericalHarmonicPolynomial<TDimension>::ComputeS(
    unsigned int from_l, unsigned int to_l, double phi, double theta
) {
  double kappa[2];
  double s;
...
template <unsigned int TDimension>
double SphericalHarmonicPolynomial<TDimension>::ComputeGaussianCurvature(
    unsigned int from_l, unsigned int to_l, double phi, double theta
) {
  double kappa[2];
...
template <unsigned int TDimension>
double SphericalHarmonicPolynomial<TDimension>::ComputeMeanCurvature(
    unsigned int from_l, unsigned int to_l, double phi, double theta
) {
  double kappa[2];
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::ComputePrincipalCurveUV(
    unsigned int from_l,
    unsigned int to_l,
    double startPhi,
    double startTheta,
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::GetOneRidgePoint(
    unsigned int from_l, unsigned int to_l, unsigned int family, double *uv, double *point
) {
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::GradKappa(
    unsigned int from_l, unsigned int to_l, unsigned int family, double *point, double *gradKappa
) {
  ...
  TopDTheta = eDTheta + 2 * lambda * fDTheta + 2 * second[1] * lambdaDTheta + lambda * lambda * gDTheta +
              2 * second[2] * lambda * lambdaDTheta;
  TopDPhi = eDPhi + 2 * lambda * fDPhi + 2 * second[1] * lambdaDPhi + lambda * lambda * gDPhi +
            2 * second[2] * lambda * lambdaDPhi;
  BottomDTheta = EDTheta + 2 * lambda * FDTheta + 2 * first[1] * lambdaDTheta + lambda * lambda * GDTheta +
                 2 * first[2] * lambda * lambdaDTheta;
  BottomDPhi = EDPhi + 2 * lambda * FDPhi + 2 * first[1] * lambdaDPhi + lambda * lambda * GDPhi +
               2 * first[2] * lambda * lambdaDPhi;
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::GetGradKappa(
    unsigned int from_l, unsigned int to_l, unsigned int family, double phi, double theta, double *dir
) {
  double uv[2], point[2];
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::FollowRidge(
    unsigned int from_l,
    unsigned int to_l,
    unsigned int family,
    double *startPoint,
...
template <unsigned int TDimension>
void SphericalHarmonicPolynomial<TDimension>::GetCaustic(
    unsigned int from_l, unsigned int to_l, int family, double phi, double theta, double *point
) {
